...
namespace FAM
{
    public class NomogrammaData  //самомтоятельный, не наследующий класс
    {
        #region КОНСТРУКТОРЫ КЛАССА

        //КОНСТРУКТОРЫ КЛАССА //////////////////////////////////////////////////////////////////////////////////        
        public NomogrammaData()
        {
        ...
        public NomogrammaData(int KQ, int NX, int MaxLines)
        {
            kp = 5;  //порядок преобразования, индекс синтезируемого ряда Фарея, допустимый порядок комбинационных частот
            TN = 1;  //тип нелинейности
            TP = 0;  //тип преобразования  ПОКА НЕ ИСПОЛЬЗУЕТСЯ
        ...
        public NomogrammaData(int arraySize)
        {
            kp = 5;  //порядок преобразования, индекс синтезируемого ряда Фарея, допустимый порядок комбинационных частот
            TN = 1;  //тип нелинейности
            TP = 0;  //тип преобразования  ПОКА НЕ ИСПОЛЬЗУЕТСЯ
        ...
        {
            k = 1;  //указывает что необходимо выполнить проверку только основных комбинационных частот
            if (tn == 1)
            {
        ...
        int i, j;  //коэффициенты счета
        int N;  //предел перебора пораженных точек
        int k = 1;  //коэффициент нескольких комбинационных частот проходящих через праженную точку    
        ...
        private void CulcPoints()
        {
            ...
            for (int i = 1; i <= nlines; i++)
            {
                ...
                {
                    Yn = 0;
                    Xn = (float)-cline[i] / aline[i];
                }
                if (Yn > 2)
                {
                    Yn = 2;
                    Xn = 2 - (float)cline[i] / aline[i];
                }
                if (Yk < 0)
                {
                    Yk = 0;
                    Xk = (float)-cline[i] / aline[i];
                }
                if (Yk > 2)
                {
                    Yk = 2;
                    Xk = 2 - (float)cline[i] / aline[i];
                }
