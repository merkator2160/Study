...
namespace EPPlusSamples
{
    class Sample15
    {
        ...
        {
            //Open Sample 1 again
            var pck = new ExcelPackage(new FileInfo(outputDir.FullName + @"\sample1.xlsx"));
            //Create a vba project             
            pck.Workbook.CreateVBAProject();
        ...
        private static void VbaSample3(DirectoryInfo outputDir)
        {
            ...
            var code = File.ReadAllText("..\\..\\VBA-Code\\CodeModule.txt");
        ...
        private static void CreateBoard(ExcelRange rng)
        {
            ...
            for (var col = 0; col <= rng.End.Column - rng.Start.Column; col++)
            {
                for (var row = 0; row <= rng.End.Row - rng.Start.Row; row++)
                {
                    if (col % 4 == 0)
                    {
