...
namespace EPPlusSamples
{
    class Sample3
    {
        ...
        public static String RunSample3(DirectoryInfo outputDir, String connectionString)
        {
            ...
            var file = outputDir.FullName + @"\sample3.xlsx";
            if (File.Exists(file)) File.Delete(file);
            var newFile = new FileInfo(outputDir.FullName + @"\sample3.xlsx");
            ...
            using (var xlPackage = new ExcelPackage(newFile))
            {
                ...
                if (worksheet != null)
                {
                    ...
                    using (var r = worksheet.Cells["A2:G2"])
                    {
                        r.Merge = true;
                    ...
                    {
                        sqlConn.Open();
                        using (var sqlCmd = new SqlCommand("select LastName + ', ' + FirstName AS [Name], EmailAddress, JobTitle, CountryRegionName, ISNULL(SalesQuota,0) AS SalesQuota, ISNULL(SalesQuota,0)*12 AS YearlyQuota, SalesYTD from Sales.vSalesPerson ORDER BY SalesYTD desc", sqlConn))
                        {
                                ...
                                while (sqlReader.Read())
                                {
                                    ...
                                    for (var i = 0; i < sqlReader.FieldCount; i++)
                                    {
                                        ...
                                        if (sqlReader.GetName(i) == "EmailAddress")
                                        {
                                            ...
                                            var hyperlink = "mailto:" + sqlReader.GetValue(i).ToString();
