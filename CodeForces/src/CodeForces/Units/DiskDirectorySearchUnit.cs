...
namespace CodeForces.Units
{
	public static class DiskDirectorySearchUnit
	{
		...
		private static DirectoryInfo[] GetAllSubDirectories(DirectoryInfo directory)
		{
			...
			{
				// Insufficient permissions, just skip this directory
			}
			catch(DirectoryNotFoundException)
			{
				// This happening from time to time, maybe because there are temp directories
			}
			catch(IOException)
			{
				// Sometimes old HDD doesn't cope with number of requests like my old Seagate
			}
