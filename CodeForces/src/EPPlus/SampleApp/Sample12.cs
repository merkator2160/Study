...
namespace EPPlusSamples
{
    ...
    public static class Sample12
    {
        ...
        public static String RunSample12(String sqlServerName, DirectoryInfo outputDir)
        {
            ...
            var file = outputDir.FullName + @"\sample12.xlsx";
        ...
        {
            var connectionStr = String.Format(@"server={0};database=AdventureWorks;Integrated Security=true;", sqlServerName);
            var ret = new List<SalesDto>();
            // lets connect to the AdventureWorks sample database for some data
            using (var sqlConn = new SqlConnection(connectionStr))
            {
                sqlConn.Open();
                using (var sqlCmd = new SqlCommand("select h.Title, FirstName, MiddleName, LastName, SubTotal, OrderDate, TaxAmt, Freight, TotalDue  from Sales.SalesOrderHeader s inner join HumanResources.Employee h on s.SalesPersonID = h.EmployeeID inner join Person.Contact c on c.ContactID = h.ContactID order by LastName, FirstName, MiddleName;", sqlConn))
                {
