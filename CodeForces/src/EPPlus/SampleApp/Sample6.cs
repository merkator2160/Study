...
namespace EPPlusSamples
{
    ...
    class Sample6
    {
        ...
        public const UInt32 ShgfiIcon = 0x100;
        public const UInt32 ShgfiLargeicon = 0x0;    // 'Large icon
        public const UInt32 ShgfiSmallicon = 0x1;    // 'Small icon
        ...
        extern static Boolean DestroyIcon(IntPtr handle);
        #endregion
        public class StatItem : IComparable<StatItem>
        {
            ...
            public Int32 CompareTo(StatItem other)
            {
        ...
        public static String RunSample6(DirectoryInfo outputDir, DirectoryInfo dir, Int32 depth, Boolean skipIcons)
        {
            _maxLevels = depth;
        ...
        private static void AddGraphs(ExcelPackage pck, Int32 rows, String dir)
        {
            ...
            var lst = new List<StatItem>(_extStat.Values);
            lst.Sort();

            //Add rows
            var row = AddStatRows(ws, lst, 2, "Extensions", "Size");
        ...
        /// <returns></returns>
        private static Int32 AddStatRows(ExcelWorksheet ws, List<StatItem> lst, Int32 startRow, String header, String propertyName)
        {
            //Add Headers
            var row = startRow;
            if (header != "")
            {
                ...
                using (var r = ws.Cells[row, 1, row, 2])
                {
                    r.Merge = true;
        ...
        private static void AlterColor(ExcelWorksheet ws, Int32 row)
        {
            using (var rowRange = ws.Cells[row, 1, row, 2])
            {
        ...
        private static Int32 AddDirectory(ExcelWorksheet ws, DirectoryInfo dir, Int32 row, Double height, Int32 level, Boolean skipIcons)
        {
            ...
            if (!skipIcons)
            {
                ...
                if (icon != null)
                {
                    var pic = ws.Drawings.AddPicture("pic" + (row).ToString(), icon);
                    pic.SetPosition((Int32)20 * (row - 1) + 2, 0);
                }
            }
            ...
            foreach (var file in dir.GetFiles())
            {
                if (!skipIcons)
                {
                    ...
                    if (fileIcon != null)
                    {
                        var pic = ws.Drawings.AddPicture("pic" + (row).ToString(), fileIcon);
                        pic.SetPosition((Int32)20 * (row - 1) + 2, 0);
                    }
                }
        ...
        private static void AddStatistics(FileInfo file)
        {
            ...
            {
                var ext = file.Extension.Length > 0 ? file.Extension.Remove(0, 1) : "";
        ...
        private static Bitmap GetIcon(String fileName)
        {
            try
            {
                ...
                var ret = SHGetFileInfo(fileName,
                                          0,
                                          ref shinfo,
                                          (UInt32)Marshal.SizeOf(shinfo),
                                          ShgfiIcon | ShgfiSmallicon);

                if (shinfo.hIcon == IntPtr.Zero) return null;

                var bmp = Icon.FromHandle(shinfo.hIcon).ToBitmap();
                DestroyIcon(shinfo.hIcon);

                //Fix transparant color 
                var invalidColor = Color.FromArgb(0, 0, 0, 0);
                for (var w = 0; w < bmp.PhysicalDimension.Width; w++)
                {
                    for (var h = 0; h < bmp.PhysicalDimension.Height; h++)
                    {
