...
namespace EPPlusSamples
{
    class Sample4
    {
        ...
        {
            using (var p = new ExcelPackage(template, true))
            {
                //Set up the headers
                ...
                using (var row = ws.Cells["A20:G20"])
                {
                ...
                using (var row = ws.Cells["A21:G21"])
                {
                    row.Style.Fill.PatternType = ExcelFillStyle.Solid;
                ...
                var file = outputdir + "\\sample4.xlsx";
